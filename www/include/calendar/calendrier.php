	...
	if ( $param['day_mode'] == 1 ) {
		...
		for ($i = 1; $i <= $nb_days_month; $i++) {
            ...
            if (!strcmp($i, date("j")) && !strcmp(date('m', $timestamp), date("m")) && !strcmp(date('Y', $timestamp), date("Y")))  {
				$output .= '<td class="calendarToday'.$param['calendar_id'].'" align="center"><a href="phpradmin.php?p=304">'.$i.$highlight.$txt_2_use.'</a></td>'."\n";// case active
			}
			elseif ($param['link_on_day'] != '') {
				...
				if (( ($param['link_after_date'] == 0) && ($current_timestamp < $loop_timestamp)) || (($param['link_before_date'] == 0) && ($current_timestamp >= $loop_timestamp)) ){
					if (file_exists($oreon->Nagioscfg->log_archive_path . "nagios-".date('m', $timestamp)."-".$i."-".date('Y', $timestamp)."-00.log"))
						$output .= '<td class="calendar'.$css_2_use.$param['calendar_id'].'"><a href="?p=304&o=d&file=nagios-'.date('m', $timestamp).'-'.$t.'-'.date('Y', $timestamp).'-00.log&date='.date('Y', $timestamp).date('m', $timestamp).date('d', $timestamp).'"><font color="red">'.$i.$highlight.$txt_2_use.'</font></a></td>'."\n";
					else if (file_exists($oreon->Nagioscfg->log_archive_path . "nagios-".date('m', $timestamp)."-"."0".$t."-".date('Y', $timestamp)."-00.log"))
						$output .= '<td class="calendar'.$css_2_use.$param['calendar_id'].'"><a href="?p=304&o=d&file=nagios-'.date('m', $timestamp).'-'."0".$t.'-'.date('Y', $timestamp).'-00.log&date='.date('Y', $timestamp).date('m', $timestamp).date('d', $timestamp).'"><font color="red">'.$i.$highlight.$txt_2_use.'</font></a></td>'."\n";
					else
	...
	if ($param['nav_link'] == 1) {
		...
		if (isset($matches[3])) $match3 = $matches[3]; else $match3 = 0;
			...
			if (file_exists($oreon->Nagioscfg->log_archive_path . "nagios-".$match1."-".$nb_day."-".$match3."-00.log"))
				$next_day_link 		= '<a href="'.$PHP_SELF.'?p=304&o=d&file=nagios-'.$match1.'-'.$nb_day.'-'.$match3.'-00.log&date='.$next_day.'" title="'.$calendar_txt[$param['lang']]['misc'][3].'">'.$d.'</a>'."\n";
			else if (file_exists($oreon->Nagioscfg->log_archive_path . "nagios-".$match1."-0".$nb_day."-".$match3."-00.log"))
				$next_day_link 		= '<a href="'.$PHP_SELF.'?p=304&o=d&file=nagios-'.$match1.'-0'.$nb_day.'-'.$match3.'-00.log&date='.$next_day.'" title="'.$calendar_txt[$param['lang']]['misc'][3].'">'.$d.'</a>'."\n";
			else
			 $next_day_link = "";
			$nb_day -=2;
			if (file_exists($oreon->Nagioscfg->log_archive_path . "nagios-".$match1."-".$nb_day."-".$match3."-00.log"))
				$previous_day_link 		= '<a href="'.$PHP_SELF.'?p=304&o=d&file=nagios-'.$match1.'-'.$nb_day.'-'.$match3.'-00.log&date='.$previous_day.'" title="'.$calendar_txt[$param['lang']]['misc'][2].'">'.$g.'</a>'."\n";
			else if (file_exists($oreon->Nagioscfg->log_archive_path . "nagios-".$match1."-0".$nb_day."-".$match3."-00.log"))
				$previous_day_link 		= '<a href="'.$PHP_SELF.'?p=304&o=d&file=nagios-'.$match1.'-0'.$nb_day.'-'.$match3.'-00.log&date='.$previous_day.'" title="'.$calendar_txt[$param['lang']]['misc'][2].'">'.$g.'</a>'."\n";
			else
				$previous_day_link = "";
			$next_month_link 	= '<a href="'.$PHP_SELF.'?p=304&o=d&file=' . $file . '&date='.$next_month.'" title="'.$calendar_txt[$param['lang']]['misc'][1].'">'.$dd.'</a>'."\n";
			$previous_month_link 	= '<a href="'.$PHP_SELF.'?p=304&o=d&file=' . $file . '&date='.$previous_month.'" title="'.$calendar_txt[$param['lang']]['misc'][0].'">'.$gg.'</a>'."\n";
