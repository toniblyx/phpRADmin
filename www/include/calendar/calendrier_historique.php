...
### French Version
$calendar_txt['fr']['monthes'] 	    = array('', 'Janvier', 'F&eacute;vrier', 'Mars', 'Avril', 'Mai', 'Juin', 'Juillet',
											'Ao&ucirc;t', 'Septembre', 'Octobre', 'Novembre', 'D&eacute;cembre');
$calendar_txt['fr']['days']		    = array('Lundi', 'Mardi', 'Mercredi','Jeudi', 'Vendredi', 'Samedi',	'Dimanche', '');
$calendar_txt['fr']['first_day']    = 0;
$calendar_txt['fr']['misc'] 	    = array('Mois pr&eacute;c&eacute;dent', 'Mois suivant','Jour pr&eacute;c&eacute;dent', 'Jour suivant');


### English version
$calendar_txt['en']['monthes']     = array('', 'January', 'February', 'March',	'April', 'May', 'June', 'July',
											'August', 'September', 'October','November', 'December');
$calendar_txt['en']['days']	    = array('Monday', 'Tuesday', 'Wednesday','Thursday', 'Friday', 'Saturday','Sunday', '');
$calendar_txt['en']['first_day']   = 0;  // put -1 to sunday first day of week
$calendar_txt['en']['misc']        = array('Previous month', 'Next month', 'Previous day', 'Next day');



### Spanish version
$calendar_txt['es']['monthes']     = array('', 'Enero', 'Febrero', 'Marzo', 'Abril', 'Mayo', 'Junio', 'Julio',
											'Agosto', 'Septiembre', 'Octubre', 'Noviembre', 'Diciembre');
$calendar_txt['es']['days']        = array('Lunes', 'Martes', 'Mi&eacute;rcoles', 'Jueves', 'Viernes', 'S&aacute;bado', 'Domingo');
$calendar_txt['es']['first_day']   = 0;
$calendar_txt['es']['misc']        = array('Mes anterior', 'Mes pr&oacute;ximo', 'd&iacute;a anterior', 'd&iacute;a siguiente');
...
function calendar($date = '') {
	...
	if ( $param['day_mode'] == 1 ) {
		...
		for ($i = 1; $i <= $nb_days_month; $i++) {
			...
			if (isset($_GET["file"])){
				preg_match("/^([0-9]{4})([0-9]{2})([0-9]{2})\.txt/", $_GET["file"], $matches);
				if (isset($matches[1]) && isset($matches[2]) && isset($matches[3]) && (!strcmp($matches[1].$matches[2].$matches[3], date('Y', $timestamp).date('m', $timestamp).$i)))
            ...
            if (!strcmp($i, date("j")) && !strcmp(date('m', $timestamp), date("m")) && !strcmp(date('Y', $timestamp), date("Y")))  {
				$output .= '<td class="calendarToday'.$param['calendar_id'].'" align="center"><a href="phpradmin.php?p=4200">'.$i.$highlight.$txt_2_use.'</a></td>'."\n";// case active
			}
			elseif ($param['link_on_day'] != '') {
				...
				if (( ($param['link_after_date'] == 0) && ($current_timestamp < $loop_timestamp)) || (($param['link_before_date'] == 0) && ($current_timestamp >= $loop_timestamp)) ){
					if (file_exists("./include/log/".date('Y', $timestamp).date('m', $timestamp).$i.".txt"))
						$output .= '<td class="calendar'.$css_2_use.$param['calendar_id'].'"><a href="?p=4200&o=d&file='.date('Y', $timestamp).date('m', $timestamp).$i.'.txt&date='.date('Y', $timestamp).date('m', $timestamp).date('d', $timestamp).'"><font color="red">'.$i.$highlight.$txt_2_use.'</font></a></td>'."\n";
					else if (file_exists("./include/log/".date('Y', $timestamp).date('m', $timestamp)."0".$i.".txt"))
						$output .= '<td class="calendar'.$css_2_use.$param['calendar_id'].'"><a href="?p=4200&o=d&file='.date('Y', $timestamp).date('m', $timestamp)."0".$i.'.txt&date='.date('Y', $timestamp).date('m', $timestamp).date('d', $timestamp).'"><font color="red">'.$i.$highlight.$txt_2_use.'</font></a></td>'."\n";
					else
	...
	if ($param['nav_link'] == 1) {
		...
		if ($param['use_img']) {
			...
			if (isset($_GET["file"]))
				...
				$file = '';
			preg_match("/^([0-9]{4})([0-9]{2})([0-9]{2})\.txt/", $file, $matches);
			...
			$nb_day++;
			if (file_exists("./include/log/".$match1.$match3.$nb_day.".txt"))
				$next_day_link = '<a href="'.$PHP_SELF.'?p=304&o=d&file='.$match3.$nb_day.$match1.'.txt&date='.$next_day.'" title="'.$calendar_txt[$param['lang']]['misc'][3].'">'.$d.'</a>'."\n";
			else
				$next_day_link = "";
			$nb_day -=2;
			if (file_exists("./include/log/".$match1.$match3.$nb_day.".txt"))
				$previous_day_link = '<a href="'.$PHP_SELF.'?p=4200&o=d&file='.$matches[3].$nb_day.$match1.'.txt&date='.$previous_day.'" title="'.$calendar_txt[$param['lang']]['misc'][2].'">'.$g.'</a>'."\n";
			else
				$previous_day_link = "";
			$next_month_link 	= '<a href="'.$PHP_SELF.'?p=4200&o=d&file=' . $file . '&date='.$next_month.'" title="'.$calendar_txt[$param['lang']]['misc'][1].'">'.$dd.'</a>'."\n";
			$previous_month_link 	= '<a href="'.$PHP_SELF.'?p=4200&o=d&file=' . $file . '&date='.$previous_month.'" title="'.$calendar_txt[$param['lang']]['misc'][0].'">'.$gg.'</a>'."\n";
