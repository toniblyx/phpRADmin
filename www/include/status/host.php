...
<table border=0 align="center">
	...
	<tr>
	 	<td class="text14b" align="center" style="text-decoration: underline; padding-bottom: 10px;"><? echo $lang['m_host_detail']; ?></td>
	</tr>
	<tr>
		<td align="center">
			<table style="border-width: thin; border-style: dashed; border-color=#9C9C9C;" cellpadding="3" cellspacing="2">
				<tr bgcolor="#eaecef">
				  ...
				  <td align="center" class="text12b">&nbsp;&nbsp;<? echo $lang['mon_ip']; ?>&nbsp;&nbsp;</td>
			...
			foreach ($Logs->log_h as $log_h)	{
				$id_host = 0;
				if ($log_h && $oreon->is_accessible($log_h->get_id())){
					if (strcmp($log_h->get_name(), $old))
						...
						echo "<tr><td>&nbsp;</td>";
					echo "<td bgcolor=" . return_color_status($log_h->get_status()) . " class='text9'' align='center'> " . $log_h->get_status() . "</td>";
					echo "<td bgcolor=". return_color($i, $log_h->get_status()) ." class='text9''> " . $oreon->hosts[$log_h->get_id()]->get_address() . "</td>";
					...
					if ($log_h->get_last_check() != 0){
						...
						echo "<td bgcolor='". return_color($i, $log_h->get_status()) ."'  class= class='text9'' align='center' style='white-space: nowrap;'> " . $hour . date(":i:s", $duration) ."&nbsp;&nbsp;</td>";
					} else
						echo "<td bgcolor='". return_color($i, $log_h->get_status()) ."'  class= class='text9'' align='center' style='white-space: nowrap;'>&nbsp;</td>";
					if (isset($_GET["type"]) && (!strcmp($_GET["type"], "0") || !strcmp($_GET["type"], "")))
							printf("<td bgcolor=" . return_color($i, $log_h->get_status()) . "><a href='phpradmin.php?p=303&o=h&type=1' class='text10' class='text9''>%.50s</a></td></tr>", $log_h->get_output());
					else
