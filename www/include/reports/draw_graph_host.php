...
*/
	...
	// Init all data
	$data = array($_GET['dataA'], $_GET['dataB'], $_GET['dataC']);
	$legend_color = array(hex2color("#".$_GET['colorA']), hex2color("#".$_GET['colorB']), hex2color("#".$_GET['colorC']));
	$legend_label = array($_GET['labelA'], $_GET['labelB'], $_GET['labelC']);
	$x = $_GET["coord_x"];
	$y = $_GET["coord_y"];
	$servicename = $_GET['sn'];
	...
	$plot->setSize(0.55, 0.9);
	$plot->set3D(10);
	$plot->setLabelPosition(5);
	$plot->explode(array(1 => 14, 2 => 20));
...
/*	$graph = new PieGraph($x,$y,"auto");
	...
	$graph->legend->SetFillColor('yellow@0.9');
	$graph->legend->Pos(0.05,0.3);
	$p1 = new PiePlot3d($data);
	$p1->SetTheme("water");
	//sand, water, earth, pastel
	$p1->SetSliceColors($legend_color);
	$p1->SetCenter(0.35	);
	$p1->SetSize(55);
	$p1->SetAngle(45);
	$p1->SetStartAngle(245);
	$p1->value->SetColor("navy");
	$p1->SetEdge("navy");
	$p1->SetLegends($legend_label);
	$p1->ExplodeAll();
	$graph->Add($p1);
	$graph->Stroke();*/
