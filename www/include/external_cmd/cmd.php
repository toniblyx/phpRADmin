...
function add_host_comment($oreon, $cmt, $lang)//phpradmin.php?p=307
{
	$check["on"] = 1;
	$check["off"] = 0;
	if (isset($cmt["pers"]))
	...
	print "<SCRIPT LANGUAGE='JavaScript'> setTimeout(\"javascript:document.location.href='phpradmin.php?p=307'\",2000)</SCRIPT>";
...
function add_host_downtime($oreon, $dtm, $lang)
{
	$check["on"] = 1;
	$check["off"] = 0;
	$duration = 60 * $dtm["dur_m"] + 3600 * $dtm["dur_h"] + 24 * 3600 * $dtm["dur_d"];
	...
	if (!isset($dtm["fixed"]))
		$dtm["fixed"] = "off";
	$str = "echo '[" . time() . "] SCHEDULE_HOST_DOWNTIME;".$dtm["host_name"].";".$start_time.";".$end_time.";".$check[$dtm["fixed"]].";".$duration.";".$dtm["auther"].";".$dtm["comment"]."' >> " . $oreon->Nagioscfg->command_file;
	system($str);
	print "<div style='padding-top: 50px' class='text11b'><center>".$lang["dtm_added"]."</center></div>";
	print "<SCRIPT LANGUAGE='JavaScript'> setTimeout(\"javascript:document.location.href='phpradmin.php?p=308'\",2000)</SCRIPT>";
}
...
function add_svc_downtime($oreon, $dtm, $lang)
{
	...
	if (!isset($dtm["fixed"]))
		$dtm["fixed"] = "off";
	$str = "echo '[" . time() . "] SCHEDULE_SVC_DOWNTIME;".$oreon->hosts[$dtm["host_id"]]->get_name().";".$dtm["service"].";".$start_time.";".$end_time.";".$check[$dtm["fixed"]].";".$duration.";".$dtm["auther"].";".$dtm["comment"]."' >> " . $oreon->Nagioscfg->command_file;
	system($str);
	print "<div style='padding-top: 50px' class='text11b'><center>".$lang["dtm_added"]."</center></div>";
	print "<SCRIPT LANGUAGE='JavaScript'> setTimeout(\"javascript:document.location.href='phpradmin.php?p=308'\",2000)</SCRIPT>";
}
...
function add_svc_hostgroup_downtime($oreon, $dtm, $lang)
{
	...
	foreach ($oreon->hostGroups[$dtm["host_group"]]->hosts as $h){
		foreach ($h->services as $s){
			$str = "echo '[" . time() . "] SCHEDULE_SVC_DOWNTIME;".$h->get_name().";".$s->get_description().";".$start_time.";".$end_time.";".$check[$dtm["fixed"]].";".$duration.";".$dtm["auther"].";".$dtm["comment"]."' >> " . $oreon->Nagioscfg->command_file;
			system($str);
			unset($s);
		}
		unset($h);
	...
	if (isset($dtm["host_too"]))
		foreach ($oreon->hostGroups[$dtm["host_group"]]->hosts as $h){
			$str = "echo '[" . time() . "] SCHEDULE_HOST_DOWNTIME;".$h->get_name().";".$start_time.";".$end_time.";".$check[$dtm["fixed"]].";".$duration.";".$dtm["auther"].";".$dtm["comment"]."' >> " . $oreon->Nagioscfg->command_file;
			system($str);
			unset($h);
		}
	print "<div style='padding-top: 50px' class='text11b'><center>".$lang["dtm_added"]."</center></div>";
...
{
	$str = "echo '[" . time() . "] DEL_HOST_DOWNTIME;".$arg."' >> " . $oreon->Nagioscfg->command_file;
...
{
	$str = "echo '[" . time() . "] DEL_SVC_DOWNTIME;".$arg."' >> " . $oreon->Nagioscfg->command_file;
	print "<div style='padding-top: 50px' class='text11b'><center>".$lang["dtm_del"]."</center></div>";
	system($str);
	print "<SCRIPT LANGUAGE='JavaScript'> setTimeout(\"javascript:document.location.href='phpradmin.php?p=308'\",2000)</SCRIPT>";
}
...
if (isset($_GET["cmd"]))
	switch ($_GET["cmd"]) {
		...
		case 45: del_host_downtime($oreon, $_GET["id"], $lang);break;//
		case 46: del_svc_downtime($oreon, $_GET["id"], $lang);break;//
