...
function setCurrentFont(){
  if (strlen($this->currentBaseFont)==0){
    ...
    $this->selectFont('./include/pdf_create/fonts/Helvetica.afm');
  }
...
function getFontHeight($size){
  if (!$this->numFonts){
    $this->selectFont('./include/pdf_create/fonts/Helvetica');
  }
...
function addText($x,$y,$size,$text,$angle=0,$wordSpaceAdjust=0){
  if (!$this->numFonts){$this->selectFont('./include/pdf_create/fonts/Helvetica');}
...
*/
function addPngFromFilewithtext($file,$x,$y,$w=0,$h=0,$text){
  ...
  if (!$error){
	$this->ezTextforimg($text,12,array('justification'=>'left'));
